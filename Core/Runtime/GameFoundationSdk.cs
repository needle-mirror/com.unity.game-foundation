    ...
    public static class GameFoundationSdk
    {
        ...
        static IEnumerator InitializeRoutine(Completer completer)
        {
            void FailInitialization(Completer aCompleter, Exception reason)
            {
                ...
                k_GFLogger.LogWarning($"{nameof(GameFoundationSdk)} failed to initialize: {reason}");
            ...
            if (!dalInitDeferred.isDone) yield return dalInitDeferred.Wait();
            ...
            try
            {
                ...
                (catalog, tags) = catalogBuilder.Build();
            }
            catch (Exception e)
            {
                const string message = nameof(GameFoundationSdk) + " failed to initialize runtime catalog from editor catalog";
                var customException = new Exception(message, e);

                FailInitialization(completer, customException);
            ...
            try
            {
                ...
                (inventory as InventoryManagerImpl).Initialize();

                wallet = new WalletManagerImpl();
                (wallet as WalletManagerImpl).Initialize();

                rewards = new RewardManagerImpl();
                (rewards as RewardManagerImpl).Initialize();

                transactions = new TransactionManagerImpl();
                (transactions as TransactionManagerImpl).Initialize();
            }
            catch (Exception e)
            {
                var customException = new Exception
                    ($"{nameof(GameFoundationSdk)} initialization failed while trying to initialize managers.", e);
